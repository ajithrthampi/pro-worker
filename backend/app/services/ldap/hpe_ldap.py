...
async def get_ldap_user(
    ...
    tls = ldap3.Tls(
        ciphers=ciphers, validate=ssl.CERT_NONE, version=ssl.PROTOCOL_TLS
    )
    ...
    if email:
        ...
        if len(nt_user_split) > 1:
            nt_user_modified = (
                f"{nt_user_split[0].upper()}:{nt_user_split[1].lower()}"
            )
        else:
    ...
    try:
        ...
        email_id = result["entries"][0]["raw_attributes"]["uid"][0].decode(
            "utf-8"
        )
        user_domain = (
        ...
        employee_id_tuple = (
            (
                result["entries"][0]["raw_attributes"]["employeeNumber"][
                    0
                ].decode("utf-8")
            ),
        )
        ...
        if "hpPictureOneHpURI" in result["entries"][0]["raw_attributes"]:
            image_url = result["entries"][0]["raw_attributes"][
                "hpPictureOneHpURI"
            ][0].decode("utf-8")
            if image_url:
...
async def validate_ldap_credentials(username: str, password: str) -> bool:
    ...
    """
    server = aioldap.Server(
        host=LDAP_AUTH_SERVER, port=LDAP_AUTH_PORT, use_ssl=True
    )
    conn = aioldap.LDAPConnection(server)
    try:
