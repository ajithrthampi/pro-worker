...
class Server(object):
    def __init__(
        ...
        if use_ssl and ssl_context:
            ...
            self.ssl_ctx = None
        if use_ssl and not tls:
            tls = ldap3.Tls(
                validate=ssl.CERT_NONE, version=ssl.PROTOCOL_TLSv1_2
            )
        self.tls = tls
...
class LDAPClientProtocol(asyncio.Protocol):
    ...
    def send(self, msg: Sequence, unbind=False) -> LDAPResponse:
        ...
        payload = encode(msg)
        logger.debug("Sending test_requests id {0}".format(msg_id))
        self.transport.write(payload)
        logger.debug("Sent test_requests id {0}".format(msg_id))
    ...
    def data_received(self, data):
        ...
        if len(data) > 0:
            ...
            while len(self.unprocessed) >= length != -1:
                logger.debug(
                    "data_received: appended msg, len: {0}".format(
                        len(self.unprocessed[:length])
                    )
                )
            ...
            while self.messages:
                ...
                try:
                    ...
                    logger.warning(
                        "data_received: Caught exception whilst decoding message"
                    )
                    continue
                ...
                if (
                    msg_asn["protocolOp"] == 1
                ):  # Bind test_requests, only 1, finished after this
    ...
    async def start_tls(self, ctx):
        ...
        ssl_proto = asyncio.sslproto.SSLProtocol(
            self.loop, self, ctx, None, server_side=False
        )
...
class LDAPConnection(object):
    ...
    async def bind(self, bind_dn: str = None, bind_pw: str = None):
        ...
        # Generate ASN1 form of LDAP bind test_requests
        ldap_msg = LDAPClientProtocol.encapsulate_ldap_message(
            msg_id, "bindRequest", bind_req
        )

        # Send test_requests to LDAP server, as multiple LDAP queries can run simultaneously,
        # were given an object to wait on which will return once done.
    ...
    async def search(
        ...
        ldap_msg = LDAPClientProtocol.encapsulate_ldap_message(
            msg_id, "searchRequest", search_req, controls=controls
        )
        ...
        return {"entries": data, "refs": resp.refs, "cookie": cookie}
    ...
    async def unbind(self):
        ...
        # asyncio.Event for the unbind test_requests is set(). As there is no response from the server, we get
    ...
    async def add(
        ...
        attributes: Dict[str, Union[List[str], str]] = None,
        controls=None,
        auto_encode: bool = True,
        timeout: Optional[int] = None,
    ):
        ...
        # So now we have attr_object_class, which contains any passed in object classes and any we've found in
        # attributes.  Converts objectclass to unicode in case of bytes value, also removes dupes
        attr_object_class = list(
            set(
                [
                    to_unicode(object_class)
                    for object_class in attr_object_class
                ]
            )
        )
